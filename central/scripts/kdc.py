...
def _get_pki_cmd() -> list[str] | None:
    """Detect available pki command (strongSwan 5.x or 6.x)."""
    # strongSwan 6.x: standalone 'pki' command
    try:
        subprocess.run(["pki", "--help"], capture_output=True, check=False)
        return ["pki"]
    except FileNotFoundError:
        pass
    # strongSwan 5.x: 'ipsec pki' wrapper
    try:
...
def has_ipsec_pki() -> bool:
    """Check if pki command is available (strongSwan 5.x or 6.x)."""
    return _get_pki_cmd() is not None


def pki_cmd() -> list[str]:
    """Return the pki command prefix."""
    cmd = _get_pki_cmd()
    if cmd is None:
        raise RuntimeError("Neither 'pki' nor 'ipsec pki' found")
    return cmd
...
class CA:
    ...
    def info(self) -> OperationResult:
        ...
        try:
            result = run(
                [*pki_cmd(), "--print", "--in", str(self.cert_path)],
                capture=True
            )
...
class CertificateManager:
    ...
    def _create_with_ipsec(
        ...
        # Get public key
        pub = run([
            *pki_cmd(), "--pub", "--in", str(private_key), "--type", "rsa",
        ], stdout=subprocess.PIPE)

        # Build issue command based on cert type
        issue_cmd = [
            *pki_cmd(), "--issue", "--lifetime", str(lifetime),
    ...
    def info(self, cert: Path | str) -> OperationResult:
        ...
        try:
            result = run(
                [*pki_cmd(), "--print", "--in", str(cert_path)],
                capture=True
            )
            info = parse_cert_info(result.stdout)
